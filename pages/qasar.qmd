...
::: callout-note
This process can be managed in GitHub desktop, VS Code, RStudio, or wherever you are most comfortable with.
:::
...
-   Ensure any files (e.g. config) required for the code are uploaded onto SharePoint for the reviewer to use. Include the SharePoint file path in the pull request from Analysis. E.g. Analysis/Public Health/Flu/Adhoc/Example Project/GitHub Files/.
...
-   Add a description of what needs to be QA'd, why changes have been made, location of any required files on Sharepoint in order to run the code, and any specific QA requests
...
If the author decides to make changes on the back of the review, the author should decide if another review is required (correcting a spelling error doesn't require one, but changing methodology may). If so, the author can use can **re-request review** on the same pull request, notifying the reviewer.
...
The author should then update the README (directly onto the main branch is fine) with QASAR information including level of QA, who conducted QA, and sign off if relevant. This information is included in the se-template and makes QASAR information easy to find. Further detail is available in the repo history.
